    ...
    <div class="w-full max-w-4xl relative z-10">
        ...
        <div class="text-center mb-8 sm:mb-12">
            <div class="inline-flex items-center justify-center glass-card rounded-full p-4 mb-6 animate-bounce-slow">
                <span class="text-4xl sm:text-5xl animate-wiggle">✏️</span>
            </div>
            <h1 class="text-3xl sm:text-4xl lg:text-5xl font-bold rainbow-text mb-4">
                ✨ Edit Your Amazing Task ✨
            </h1>
            <p class="text-white/80 text-lg sm:text-xl max-w-2xl mx-auto">
                🚀 Update your task with style and make it even more awesome! 🌟
            </p>
        ...
        <div class="form-container glass-card rounded-2xl p-6 sm:p-8 lg:p-12 border border-white/20">
            <!-- Error Messages -->
            @if ($errors->any())
                <div class="glass-card border-l-4 border-red-400 p-5 sm:p-6 mb-8 rounded-xl">
                    <div class="flex items-center mb-4">
                        <div class="bg-red-500 p-2 rounded-full mr-4 animate-bounce">
                            <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                        </div>
                        <h3 class="text-white text-lg font-semibold">🚨 Oops! Please fix these issues:</h3>
                    </div>
                    <ul class="list-disc list-inside text-white/90 space-y-2">
                        @foreach ($errors->all() as $error)
                            <li class="text-base sm:text-lg">{{ $error }}</li>
                        @endforeach
                    </ul>
                </div>
            @endif
            ...
            <form action="{{ route('tasks.update', $task->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="input-group">
                    ...
                    <textarea name="description" id="description" required rows="6"
                              ...
                              class="glass-input w-full px-5 py-4 rounded-xl text-lg sm:text-xl transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-rainbow-green/50 resize-y">{{ $task->description }}</textarea>
                </div>
                ...
                <div class="input-group">
                    ...
                    <select name="status" id="status" 
                        ...
                        <option value="Not Started" {{ $task->status == 'Not Started' ? 'selected' : '' }} class="bg-gray-800 text-white">⏳ Not Started</option>
                        <option value="Doing" {{ $task->status == 'Doing' ? 'selected' : '' }} class="bg-gray-800 text-white">🔄 Doing</option>
                        <option value="Done" {{ $task->status == 'Done' ? 'selected' : '' }} class="bg-gray-800 text-white">✅ Done</option>
                    </select>
                </div>
                ...
                <div class="input-group">
                    ...
                    @if($task->image)
                        <div class="mt-6">
                            <p class="text-white font-semibold text-lg mb-4 flex items-center">
                            ...
                            <div class="relative group">
                                <img src="{{ asset('storage/' . $task->image) }}" 
                                     ...
                                     class="w-full max-w-md h-64 object-cover rounded-xl shadow-2xl border-4 border-white/30 transition-all duration-500 group-hover:scale-105 group-hover:rotate-1">
                                <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-rainbow-blue/30 opacity-0 group-hover:opacity-100 transition-all duration-300 rounded-xl flex items-end justify-center p-4">
                                    <span class="text-white font-bold text-lg">Current Image</span>
                                </div>
                            </div>
                        </div>
                    @endif
